...
用法:
    python -m data_infra.scripts.check_data                        # 运行巡检
    python -m data_infra.scripts.check_data --symbols BTC/USDT     # 只检查指定币对
    python -m data_infra.scripts.check_data --fix                  # 巡检并自动修复 K线缺口
"""
...
from data_infra.data.validator import check_kline_continuity, check_tick_sync_status
...
def main():
    ...
    if args.fix:
        print("\n--- 修复提示 ---")
        print("  Tick 缺失: 请手动执行 python -m data_infra.scripts.backfill --type tick --from-id <id>")
